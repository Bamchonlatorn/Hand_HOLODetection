...
video_files = ['Bluewhale_output.mp4', 'BlueJellyfish_output.mp4']
videos = [cv2.VideoCapture(file) for file in video_files]
video_names = ['jellyfish', 'fishtank']
...
fingers_time_count = [0, 0]  # To store the time count for 1 to 8 fingers
    ...
    if result.multi_hand_landmarks:
        ...
        if total_fingers in [1, 2]:
            ...
            if fingers_time_count[total_fingers - 1] >= required_seconds * 2:  # Assuming 30 FPS
                ...
                fingers_time_count = [0, 0]  # Reset the counts
        else:
            fingers_time_count = [0, 0]  # Reset if other than 1 to 8 fingers

        if last_direction and selected_video is not None:
